...
int solution(int k, int curr)
{
	if (k == 0) return (curr+1) % 2;
	...
	for (int i = 0; i<n; i++)
	{
		winner = solution(k-arr[i], (curr+1) % 2);
		if (winner == curr) return dp[k][curr] = curr;
	}

	return dp[k][curr] = winner;
}
...
int main() 
{
    ...
    for (int i=0; i<100001; i++) dp[i][0] = dp[i][1] = -1;
	...
	cout << (solution(k, 0) == 0 ? "First" : "Second");

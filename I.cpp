...
int main() 
{
    ...
    int n; cin >> n;
    vector<double> odds(n + 1, 0);

    for (int i=1; i<=n; i++) cin >> odds[i];

    // state dp[i][j] probability of getting i heads and j tails

    // answer sum of of all i, j where i + j == n and i > j
    ...
    for (int i=0; i<n; i++)
    	...
    	{
    		// next coin tosses a head
    		dp[i+1][j] += dp[i][j] * odds[i+j+1];

    		// next coin tosses a tail
    		dp[i][j+1] += dp[i][j] * (1 - odds[i+j+1]);

...
int main() 
{
    ...
    vl weights(n+1), values(n+1);

    for (int i=1; i<=n; i++) cin >> weights[i] >> values[i];

    vector < vl > dp(n+1, vl(100005, INT_MAX));
	
	// state :
	// dp[i][k] -> minimum weight incurred to obtain k total value using first i items
	...
	for (int i=0; i<n; i++)
	{
		...
		{
			// did not take the i'th element
			dp[i+1][j] = min(dp[i+1][j], dp[i][j]);

			// take the i'th element
			if (j+values[i+1] <= 100005)
				dp[i+1][j+values[i+1]] = min(dp[i+1][j+values[i+1]], dp[i][j] + weights[i+1]);
		}
	}

	ll ans = 0;
	for (int i=0; i<=100000; i++) if (dp[n][i] <= w)
		ans = i;

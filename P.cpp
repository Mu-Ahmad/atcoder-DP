...
{
	int all_white = 1; // means current vertex can be painted black
	int sth_black = 0;

	for (auto v : adj_list[curr]) if (v != parent)
	{
		auto p = dfs(v, curr); 
		// p.first is the number of ways to color subtree such that b is black

		sth_black = (mul(all_white, p.fi) + mul(sth_black, p.fi+p.se)) % mod;

		all_white = mul(all_white, p.se);
	}

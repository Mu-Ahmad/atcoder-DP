...
int n;
ll dp[3001][3001][2];
bool computed[3001][3001][2];

ll solve(int l, int r, int curr)
{
	if (l == r) return arr[l];

	if (computed[l][r][curr]) return dp[l][r][curr];

	computed[l][r][curr] = true;

	return dp[l][r][curr] = max 
		   ( 
		   	arr[l] - solve(l+1, r, (curr+1) % 2),
		    arr[r] - solve(l, r-1, (curr+1) % 2)
		   );
}
...
int main() 
{
    ...
    for (int i=0; i<3001; i++)
    	for (int j=0; j<3001; j++) 
    		computed[i][j][0] = computed[i][j][1] = false;
